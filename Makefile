...
./build/imgs/boot.img: $(bin_dir)/loader.bin $(bin_dir)/kernel.bin
	#dd if=./build/bins/boot.bin of=./build/imgs/boot.img count=1 seek=1 conv=notrunc
	#dd if=./build/bins/boot.bin of=./build/imgs/boot.img count=1 seek=6 conv=notrunc
	@mkdir -p ./mnt
